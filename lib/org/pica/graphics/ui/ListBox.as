	...
	public class ListBox extends Sprite {
		...
		public function ListBox(name:String, itemWidth:Number, itemHeight:Number, itemCount:int, onSelected:Function, align:int=ListBox.ALIGN_VERTICAL) {
			...
			btnNext_ = new Button(name + "_next", null, [isVertical?icons_[1]:icons_[3]], { up:onButton }, isVertical?0:btnPrev_.logicalWidth+itemWidth_*itemCount_, isVertical?btnPrev_.logicalHeight+itemHeight_*itemCount_:0, 
				isVertical?itemWidth_:-1, isVertical?-1:itemHeight_,
				Button.ICON_LEFT, btnTheme_);
			btnNext_.disable = true;
			btnPrev_.disable = true;
			addChild(btnPrev_);
			addChild(btnNext_);
		}
		...
		public function addItem(name:String, label:String, icon:Bitmap):Button {
			var b:Button = new Button(name, label, icon?[icon]:null, { up:onButton }, 0, 0, itemWidth_, itemHeight_, isVertical?Button.ICON_LEFT:Button.ICON_UP, btnTheme_);
			b.toggle = true;
		...
		public function removeItem(name:String):void {
			...
			lastFirst_ = items_.length - displayItems_.length;
			var i:int = 0;
			for each (var btn:Button in items_) {
				if (btn.name == name) {
					if (selItem_ && selItem_ == btn) selItem_ = null;
					items_.splice(i, 1);
					return;
				}
				i++;
		...
		public function getItem(name:String):Button {
			return this.getChildByName(name) as Button;
		}
		
		public function selectedItemByName(name:String):void {
			selectedItem(getItem(name));
			return;
		}
		
		public function selectedItem(btn:Button):void {
			if (btn == null) return;
			if (selItem_) selItem_.isOn = false;
			selItem_ = btn;
			selItem_.isOn = true;
		}
		...
		private function onButton(btn:Button):void {
			...
			default:
				if (onSel_ != null) {
					if (btn == selItem_) { btn.isOn = true; return; }

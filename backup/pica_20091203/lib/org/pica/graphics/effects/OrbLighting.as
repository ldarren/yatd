package org.pica.graphics.effects {
	...
	public class OrbLighting extends Orb{
		...
		static public const EFFECT_RADIUS:uint = 4096;
		...
		public function update():DisplayObject3D {
			if (isPoweredOn()) {
				...
				if (targetObj_ != null) {
					if (isTarget(targetObj_.position)) {
		...
		private function isTarget(pos:Number3D):Boolean {
			if (pos == null) return false;
			var dx:Number = pos.x - this.x; var dz:Number = pos.z - this.z;
			if ((dx * dx + dz * dz) < OrbLighting.EFFECT_RADIUS) return true;
			return false;
		}
